# Create list of students in global scope to be used
students = [{"name": "Art", "hometown": "Detroit", "favorite_food": "Pasta"},
  {"name": "Bryce", "hometown": "Lansing", "favorite_food": "Rice"},
  {"name": "Alex", "hometown": "Chicago", "favorite_food": "Burrito"}
]

# Define function to print the name and index number of each student
def list_names(student_list):
  # list each student, printing name and position in the provided list
  for student in student_list:
    print(str((student_list.index(student)+1)) + ". " + student.get("name"))

def prompt():
  # Add, view, or quit options for user
  while True:
